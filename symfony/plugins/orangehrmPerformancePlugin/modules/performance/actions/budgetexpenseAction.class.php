 ...
 *
 * */
class budgetexpenseAction extends basePeformanceAction {
 ...
 public function execute($request){
     ...
     //die($id."dsd");
   $this->pager = new sfDoctrinePager(OhrmBudgetExpensesTable::getInstance(), sfConfig::get('app_max_jobs_on_homepage'));
        $this->pager->setQuery(Doctrine::getTable('OhrmBudgetExpenses')->createQuery('a')->where('a.budget = ?',$id)->orderBy('a.id DESC'));
    $this->pager->setPage($request->getParameter('page', 1));
    $this->pager->init();
    $this->budget=  OhrmBudgetsTable::getBudget($id);
