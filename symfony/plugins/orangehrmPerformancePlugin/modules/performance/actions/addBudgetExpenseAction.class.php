 ...
 *
 * */
class addBudgetExpenseAction extends basePeformanceAction {
 ...
 public function execute($request) {
     ...
     $form=new BudgetExpenseForm(array(), $optionsForForm, true);
              $this->setForm($form);
             ...
             // die(print_r($postArray));
                ...
                $_SESSION['budgetexpense'] = $postArray;
                     $this->form->bind($request->getParameter($this->form->getName()));
    if ($this->form->isValid()) {
                    $trainingcourses=new OhrmBudgetExpenses();
                   $trainingcourses->setBudget($this->form->getValue('budget'));
                  $trainingcourses->setExpenseCategory($this->form->getValue('expense_category'));
                $trainingcourses->setAmount($this->form->getValue('amount'));
                $trainingcourses->setDescription($this->form->getValue('description'));
                   $trainingcourses->setRequestedBy($this->form->getValue('requested_by'));
                   $trainingcourses->setApprovedBy($this->form->getValue('approved_by'));
                   
             $trainingcourses->save();
                    
                       $this->getUser()->setFlash('success', __('Successfully saved expense'));
                        $this->redirect('performance/budgetssetup?id='.$this->form->getValue('budget'));
