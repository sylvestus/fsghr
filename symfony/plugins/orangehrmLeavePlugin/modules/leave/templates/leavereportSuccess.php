...
<div class="box searchForm toggableForm" id="employee-information">
    <div class="head">
        <h1><?php echo __('Leave usage and Entitlements'); ?>&nbsp;&nbsp; </h1>
    ...
    <div class="inner">
        ...
        <a href="#" class="addbutton" style="float:right !important" id="export"><img src="<?php echo "{$imagePath}/excelicon.png"; ?>" /></a>
              ...
              <input type="hidden" id="assignencashment" value="<?=  url_for('payroll/encashment')?>">
               <input type="hidden" id="removeencashment" value="<?=  url_for('payroll/deleteEncashment')?>">
              <br>
            </p>
            <div id="dvData">
               ...
               if(count($record)>0){
                    ...
                    foreach ($record as $value) {
                      $empno=$value["emp_number"]  ;
                       ...
                       <td class="tdValue"><?=$leaveentitlement-$leavetaken-$leaveencashed?></td>
                       <td style="text-align:right"><input type="number"  title="<?=$empno?>" style="width:50px;border:1px solid #ccc;border-radius:5px" id="daysencash<?=$empno?>" value="<?=$leaveentitlement-$leavetaken-$leaveencashed?>" max="<?=$leaveentitlement-$leavetaken-$leaveencashed?>" min="0">&nbsp;<input type="checkbox" class="checkboxAtch" title="#daysencash<?=$empno;?>" name="chkListRecord[]" value="<?=$empno;?>"></td>
                    </tr>
...
$(function() {
    ...
    $('.checkboxAtch').click(function(e) {
        ...
        // e.preventDefault();
           if ($(this).is(":checked")) {
               ...
               // alert(days)
         var url=$("#assignencashment").val();
           
       //  window.location.replace(url+'?id='+id);
    var jqxhr = $.post(url,{ emp:id, days:days,assign:true }, function() {
  alert( "saving..." );
})
  .done(function() {
    alert( "success" );
  ...
  .fail(function() {
    alert( "error in processing!" );
              ...
              var url=$("#removeencashment").val();
       var jqxhr = $.post(url,{ emp:id, days:days,assign:false }, function() {
  alert( "deleting..." );
})
  .done(function() {
    alert( "success" );
