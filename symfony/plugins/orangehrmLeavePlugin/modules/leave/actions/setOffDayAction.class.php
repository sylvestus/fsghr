 ...
 * changeLeaveStatus
 */
class setOffDayAction extends baseLeaveAction {
    ...
    public function execute($request) {
           ...
           $day= $request->getParameter('day');
            $empno=$request->getParameter('emp');
              $q = Doctrine_Manager::getInstance()->getCurrentConnection();
            if($status==1){
               
           $pdo = $q->execute("SELECT * from offdays where off_day='$day' and employee_id='$empno'");
$pdo->setFetchMode(Doctrine_Core::FETCH_ASSOC);
$offdaydetails=$pdo->fetchObject();
                if(is_object($offdaydetails)){
                 $pdo = $q->execute("DELETE from offdays where off_day='$day' and employee_id='$empno'"); 
                   echo "Off day removed";
                }
                else{
                $offday=new Offdays();
                $offday->setActive(1);
                $offday->setEmployeeId($empno);
                $offday->setOffDay($day);
                $offday->save();
                echo "Off day set";
                }
                //insert
