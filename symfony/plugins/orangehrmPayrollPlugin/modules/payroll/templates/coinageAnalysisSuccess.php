...
<div class="box searchForm toggableForm" id="employee-information">
    ...
    <div class="head">
        <h1><?php echo __('Coinage Analysis  ('.$month.')'); ?>&nbsp;&nbsp; </h1>
            
       <?php $imagePath = theme_path("images");?>     

    </div>
    <div class="inner">
         <form id="search_form" name="frmEmployeeSearch" method="post" action="<?php echo url_for('payroll/coinageAnalysis'); ?>">
    ...
    foreach ($denominations as $key=>$value) {
        while ($amount >= $value) {
            $amount = round($amount - $value,0); // decrement by the value of the denomination
            if (!isset($count[$key])) {
                $count[$key] = 1;
            } else {
    ...
    foreach ($count as $key=>$value) {
        $breakdown[] = '<span class="boldText">'."$value".'</span>&nbsp;('."$key".')'; // combine the keys with the value for formatting
    }
    return implode (", ",$breakdown); // combine the array into a comma-delineated string
     ...
     <br>
            ...
            <div id="dvData">
                ...
                <thead>
                    <tr><th></th><th class="boldText"><?php echo __(strtoupper($organisationinfo->getName())); ?></th><th></th><th></th></tr>
                  
                    <tr><th></th><th class="boldText"><?php echo __(strtoupper('Coinage Analysis For Payroll Period  ('.$month.')')); ?></th><th></th><th></th></tr>
                    <tr>
                       
                        <th><?php echo __('Employee No'); ?></th>
                        <th><?php echo __('Name'); ?></th>
                         <th><?php echo __('Net Pay'); ?></th>
                            <th><?php echo __('Denominations'); ?></th>
                    </tr>
                </thead>
                <tbody>
                    ...
                    ?>
                     <?php
                       ...
                       <?= $record->getEmpname()?>    
                        ...
                        <td class="tdValue"><?php echo breakdownDenominations(round($record->getNetPay()), $denominations) ?></td>
                    </tr>
...
$(document).ready(function() {
    $("#emailbtn").click(function(e){
     html=$("#pdftable").html();
     url=$("#pdfurl").val()+"?sendemail=true&report=Coinage Analysis";
$.ajax({
            url: url,
            type: 'post',
            data: {
