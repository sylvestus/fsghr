...
<div class="box searchForm toggableForm" id="employee-information">
    ...
    <div class="inner">
         <form id="search_form" name="frmEmployeeSearch" method="post" action="<?php echo url_for('payroll/departmentregister'); ?>">
    ...
    <div class="inner">
     ...
     <input type="button" class="addbutton print" style="float:right !important" id="btnAddSlab" rel="dvData"  value="<?php echo __('Print') ?>"/>
      <!--<input type="button" class="addButton" style="float:right !important" id="btnAddSlabt" value="<?php echo __('Accounting Post'); ?>"/>-->
     <br>
            </p>
            <div id="dvData">
            ...
            <?php echo __(strtoupper('Department Register for  '.$month)); ?>&nbsp;&nbsp;<br> </th></tr>
                   
               <?php
               //check for employee loans this month
                    ...
                    <tr>
                        ...
                        <th  class="borderBottom" style="text-align:right" width="20%"><?php echo __('Name'); ?></th>
                                ...
                                <th  class="borderBottom" style="text-align:right" width="6%"><?php echo __('Loan Repaid'); ?></th>
                                   <th  class="borderBottom" style="text-align:right" width="5%"><?php echo __('Loan Int'); ?></th>
                                   <?php }?>
                     ...
                     <td  class="tdValue"  style="text-align:right;" width="3%">
                       <?= $count?>    
                           ...
                           <td class="tdValue" width="8%">
                            <?php 
                           $employeedetail=  EmployeeDao::getEmployeeByNumber($allslips[$i]->getEmpNumber());
...
//                        $allinterests=$allinterests+$interest;
           ...
           <?php
           $employeedetaill=  EmployeeDao::getEmployeeByNumber($allslips[$i]->getEmpNumber()); 
           $location=$employeedetaill->work_station;
           $department=  OhrmSubunitTable::getDepartment($location);
            array_push($locationids,$employeedetail->work_station); //track locations being used
           $futurei=$i+1;
           if($futurei < count($allslips)){
           $empdetail=EmployeeDao::getEmployeeByNumber($allslips[$futurei]->getEmpNumber());
           $location1=$empdetail->work_station;
            }
           if($location !=$location1){ 
           $subtotal=  payslipDao::getPayslipsForDepartment($location,$monthyear);
           $border="border-bottom:1px solid #000;border-top:1px solid #000;font-weight:bold;text-align:right"; 
               ?>
          <tr><td class="boldText tdValue" style="<?=$border?>"></td><td class="boldText" style="<?=$border?>"><?=$department->name?></td><td class="boldText" style="<?=$border?>"></td><td class="boldText" style="<?=$border?>"><?=  number_format($subtotal["total_basic"])?></td><td class="boldText" style="<?=$border?>"><?=  number_format($subtotal["total_housing"])?></td><td class="boldText" style="<?=$border?>"><?=number_format($subtotal["total_gross"])?></td><td class="boldText" style="<?=$border?>"><?=  number_format($subtotal["total_nssf"])?></td><td class="boldText" style="<?=$border?>"><?=  number_format($subtotal["total_nhif"])?></td><td class="boldText" style="<?=$border?>"><?=  number_format($subtotal["total_payee"])?></td><?php if($mps>5){?><td class="boldText" style="<?=$border?>"><?=  number_format($subtotal["total_loan"])?></td><td class="boldText" style="<?=$border?>"><?= number_format($subtotal["total_interest"])?></td><?php }?><td class="boldText" style="<?=$border?>"><?=  number_format($subtotal["total_deductions"])?></td><td class="boldText" style="<?=$border?>"><?=  number_format($subtotal["total_net"])?></td></tr>       
              ...
              <?php   
              $unique=array_unique($locationids);
              $lastlocation=end($unique);
              $lastsubtotal=  payslipDao::getPayslipsForDepartment($lastlocation, $monthyear);
              $dept= OhrmSubunitTable::getDepartment($lastlocation);
              $locationname=$dept->name;
              ?>
              <tr><td class="boldText" style="<?=$border?>"><br></td><td class="boldText" style="<?=$border?>"><?=$locationname?></td><td class="boldText" style="<?=$border?>"></td><td class="boldText" style="<?=$border?>"><?=  number_format($lastsubtotal["total_basic"])?></td><td class="boldText" style="<?=$border?>"><?=  number_format($lastsubtotal["total_housing"])?></td><td class="boldText" style="<?=$border?>"><?=number_format($lastsubtotal["total_gross"])?></td><td class="boldText" style="<?=$border?>"><?=  number_format($lastsubtotal["total_nssf"])?></td><td class="boldText" style="<?=$border?>"><?=  number_format($lastsubtotal["total_nhif"])?></td><td class="boldText" style="<?=$border?>"><?=  number_format($lastsubtotal["total_payee"])?></td><?php if($mps>5){?><td class="boldText" style="<?=$border?>"><?=  number_format($lastsubtotal["total_loan"])?></td><td class="boldText" style="<?=$border?>"><?= number_format($lastsubtotal["total_interest"])?></td><?php }?><td class="boldText" style="<?=$border?>"><?=  number_format($lastsubtotal["total_deductions"])?></td><td class="boldText" style="<?=$border?>"><?=  number_format($lastsubtotal["total_net"])?></td></tr>       
             
              <tr><td class="boldText" colspan="3" style="font-weight:bold">Total Employees: <?=$count-1?></td><td class="boldText" style="font-weight:bold;text-align:right"><?= number_format(array_sum($totalbasic),0)?></td><td class="boldText" style="font-weight:bold;text-align:right"><?=  number_format(array_sum($totalhouse),0)?></td><td class="boldText" style="font-weight:bold;text-align:right"><?=  number_format(array_sum($totalgross),0)?></td><td class="boldText" style="font-weight:bold;text-align:right"><?=  number_format(array_sum($totalnssf),0)?></td><td class="boldText" style="font-weight:bold;text-align:right" ><?=  number_format(array_sum($totalnhif),0)?></td><td class="boldText" style="font-weight:bold;text-align:right"><?=  number_format(array_sum($totalpaye),0)?></td><?php if($mps>5){?><td class="boldText" style="font-weight:bold;text-align:right"><?=  number_format(array_sum($totalloan),0)?></td><td class="boldText" style="font-weight:bold;text-align:right"><?= number_format(array_sum($totalinterest),0)?></td><?php }?><td class="boldText" style="font-weight:bold;text-align:right"><?=  number_format(array_sum($totaldeductions),0)?></td><td class="boldText" style="font-weight:bold;text-align:right"><?=number_format(array_sum($totalnet),0)?></td></tr>         
                </tbody>     
