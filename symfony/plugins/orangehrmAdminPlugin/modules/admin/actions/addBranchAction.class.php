 ...
 */
class addBranchAction extends baseAdminAction {
    ...
    protected function getUndeleteForm() {
        return new UndeleteCustomerForm(array(), array('fromAction' => 'addBranch'), true);
    }
    ...
    $earnings=new OhrmBankBranch();
        $earnings->setBankId($this->form->getValue('bank_id'));
    $earnings->setBranchName($name);
    $earnings->setBranchCode($this->form->getValue('branch_code'));
   
    $earnings->save();
    $this->getUser()->setFlash('success', __('Successfully added branch: '.$name));
