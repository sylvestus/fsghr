...
class saveSystemUserAction extends sfAction {
    ...
    public function execute($request) {
        ...
        $user = $this->getSystemUserService()->getSystemUser($this->userId);
                    if ($user instanceof SystemUser) {
                    $newRoleId = $user->getUserRoleId();
                    }
                    $roles=OhrmUserRoleScreenTable::getUserRoleScreens($newRoleId);       
                    $this->roles=$roles;
                    
        $values = array('userId' => $this->userId, 'sessionUser' => $this->getUser()->getAttribute('user'));
        $this->setForm(new SystemUserForm(array("roles"=>$roles), $values));
