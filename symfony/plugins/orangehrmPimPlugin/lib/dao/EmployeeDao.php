...
class EmployeeDao extends BaseDao {
    ...
    protected static $sortMapping = array(
     ...
     public static function hydrateEmployeeEarnings() {
        try {
$employees= EmployeeTable::getInstance()->findByDql("termination_id is NULL order by emp_firstname ASC");
foreach ($employees as $employee) {
    $choices[$employee->getEmpNumber()]=$employee->getEmpFirstname().' '.$employee->getEmpMiddleName().' '.$employee->getEmpLastname();
}
return $choices;
        } catch (Exception $e) {
...
$employees= EmployeeTable::getInstance()->findAll();
$employeesinlocation= HsHrEmpLocationsTable::findEmployeesInLocation($location);

foreach ($employees as $employee) {
    if(in_array($employee->getEmpNumber(),$employeesinlocation)){
    $choices[$employee->getEmpNumber()]=$employee->getEmpFirstname().' '.$employee->getEmpMiddleName().' '.$employee->getEmpLastname();
}
}
return $choices;
     ...
     */
     ...
     public static function getEmployeesOnProbation() {
      
        
             $q = Doctrine_Query:: create()->from('HsHrEmployee ec')
                     ->where('ec.termination_id is NULL') 
                     ->andWhere('ec.on_probation = 1') //added for terminated employees not to show
                                    ->orderBy('ec.employee_id ASC');
            return $q->execute();
           
       
    }
    
      public static function getEmployeesProbationUnsuccessful() {
       $q = Doctrine_Query:: create()->from('HsHrEmployee ec')
                     ->where('ec.termination_id is NULL') 
                     ->andWhere('ec.on_probation = 2') //added for terminated employees not to show
                                    ->orderBy('ec.employee_id ASC');
            return $q->execute();
    ...
    public static function getEmployees($limit=0,$offset=0){
       try {
       if(!$limit){
            $q = Doctrine_Query:: create()->from('HsHrEmployee ec')
                     ->where('ec.termination_id is NULL') //added for terminated employees not to show
                                    ->orderBy('ec.employee_id ASC');
           }else{
               $q = Doctrine_Query:: create()->from('HsHrEmployee ec')
                     ->where('ec.termination_id is NULL') //added for terminated employees not to show
     ...
     /*************get all employees in order**************/
    public static function getAllEmployeesPayroll($limit=0,$offset=0){
       try {
       if(!$limit){
            $q = Doctrine_Query:: create()->from('HsHrEmployee ec')
                     //->where('ec.termination_id is NULL') //added for terminated employees not to show
    ...
    public static function getEmployeesMinInfo($limit=0,$offset=0){
       try {
       if(!$limit){
            $q = Doctrine_Query:: create()->from('HsHrEmployee ec')
                    ->select('ec.emp_number,ec.emp_gender,ec.employee_id,ec.emp_firstname,ec.emp_middle_name,ec.emp_lastname,ec.job_title_code,ec.work_station,ec.termination_id,ec.joined_date')
                    // ->where('ec.termination_id is NULL') //added for terminated employees not to show
                                    ->orderBy('ec.emp_firstname ASC');
           }else{
               $q = Doctrine_Query:: create()->from('HsHrEmployee ec')
                     ->where('ec.termination_id is NULL') //added for terminated employees not to show
                                    ->orderBy('ec.employee_id ASC')
    ...
    public static function getAllEmployees($limit=500,$offset=0){
      ...
      //  die("Select * from hs_hr_employee hr WHERE hr.termination_id IS NULL LIMIT $limit OFFSET $offset");
      return $q->execute(array(),Doctrine::HYDRATE_ARRAY);

    }
    
    /*public static function getAllEmployeesPerLocation($limit=500,$offset=0,$location){
    
        $q = Doctrine_Query:: create()->from('HsHrEmployee ec')
                    ->select('e1.emp_number,ec.emp_gender,ec.employee_id,ec.emp_firstname,ec.emp_middle_name,ec.emp_lastname,ec.job_title_code,ec.work_station,ec.termination_id,ec.joined_date')
                ->innerJoin('HsHrEmpLocations el ON ec.emp_number=el.emp_number')
                              ->where('el.location_id = ?', $location)
                ->orderBy('ec.employee_id ASC')
               ->offset($offset)
                       ->limit($limit)
                     ->where('ec.termination_id is NULL'); //added for terminated employees not to show
	...
	*/
	...
	public static function  getAllEmployeesPerLocation($limit=500,$offset=0,$location){
		$completeQuery="SELECT ec.emp_number,ec.emp_gender,ec.employee_id,ec.emp_firstname,ec.emp_middle_name,ec.emp_lastname,ec.job_title_code,ec.work_station,ec.termination_id,ec.joined_date from hs_hr_employee ec JOIN `hs_hr_emp_locations` on ec.emp_number=hs_hr_emp_locations.emp_number WHERE hs_hr_emp_locations.location_id=? AND ec.termination_id IS NULL";
		$conn = Doctrine_Manager::connection();
        $statement = $conn->prepare($completeQuery);
        $result = $statement->execute(array($location)); //,$limit,$offset
  ...
  public static function getDuplicateEmployees($limit=500,$offset=0){
        $conn = Doctrine_Manager::connection();
        $pdo = $conn->execute("Select emp_number,employee_id,COUNT(employee_id),emp_lastname,emp_firstname,emp_middle_name,fingerprint,photo from hs_hr_employee hr GROUP BY(employee_id) HAVING COUNT(employee_id) > 1 ORDER BY employee_id ASC LIMIT $limit OFFSET $offset " ); //WHERE hr.termination_id IS NULL
      //  die("Select * from hs_hr_employee hr WHERE hr.termination_id IS NULL LIMIT $limit OFFSET $offset");
    ...
    /*******************check joined date hence current month pay***************/
       public static function checkJoinedDate($joineddate,$monthyear)
{
$firstday="1-".$monthyear;
$date= DateTime::createFromFormat("d-m-Y", $firstday);
$firstday=$date->format("d-m-Y");
$month=$date->format("m");
$daysinmonth=  self::getDaysinMonth($month);
//if employee joined in this month
if(date("m-Y",strtotime($joineddate))==date("m-Y",strtotime($firstday))){
...
/*******get Employee By id Search*****************/
      public static function getEmployeeByNumber($empNumber){
        
       try {
            $q = Doctrine_Query:: create()->from('HsHrEmployee ec')
                      ->where('ec.emp_number = ?', $empNumber)
                                    ->orderBy('ec.emp_firstname ASC,ec.emp_middle_name ASC');
             $q->execute();
             return $q->fetchOne();
        // @codeCoverageIgnoreStart
        } catch (Exception $e) {
    ...
    public static function getEmployeeMinInfoByNumber($empNumber){
        
       try {
            $q = Doctrine_Query:: create()->from('HsHrEmployee ec')
                    ->select('ec.emp_number,ec.emp_gender,ec.employee_id,ec.emp_firstname,ec.emp_middle_name,ec.emp_lastname,ec.emp_birthday,ec.job_title_code,ec.work_station,ec.termination_id,ec.joined_date,ec.custom1 as paygrade')
                      ->where('ec.emp_number = ?', $empNumber)
                                    ->orderBy('ec.emp_firstname ASC,ec.emp_middle_name ASC');
             $q->execute();
             return $q->fetchOne();
    ...
    public function getEmployeesByDepartment($subUnits, $includeTerminatedEmployees = false) {
        try {
    ...
    /**********Employees in certain location/branch************/
        public function getEmployeesByBranch($location) {

        try {
            if(intval($location)>0){
            $q = Doctrine_Query:: create()->from('HsHrEmployee hs')
                              ->leftJoin('hs.HsHrEmpLocations el ON hs.emp_number=el.emp_number')
                              ->where('el.location_id = ?', $location)
                              ->orderBy('hs.employee_id ASC');
            }
            else{
             $q = Doctrine_Query:: create()->from('HsHrEmployee hs')
                            ->leftJoin('hs.HsHrEmpLocations el ON hs.emp_number=el.emp_number')
    ...
    public function getEmployeeIdList($excludeTerminatedEmployees = false) {
     ...
     public function getEmployeeIdListByPaymentType($excludeTerminatedEmployees = false,$paymenttype) {
        ...
        try {
                ...
                $employeeIds = $q->execute(array(), Doctrine_Core::HYDRATE_SINGLE_SCALAR);
                 ...
                 $query= Doctrine_Query :: create()
                            ->select('e.empnumber')
                            ->from('HsHrEmpdefaultpaymode e')
                            ->where("`paymentmethod`='$paymenttype'");
                  $employeeIdspayment = $query->execute(array(), Doctrine_Core::HYDRATE_SINGLE_SCALAR);
                ...
                if (is_string($employeeIds)) {
                 ...
                 if (is_string($employeeIdspayment)) {
                    $employeeIdspayment = array($employeeIdspayment);
                    
                }
               
                //retun only those ids in payment type
                $sanitisedempids=array();
                foreach ($employeeIds as $id) {
                    if(in_array($id, $employeeIdspayment)){
                        array_push($sanitisedempids, $id);
                    }
                }
                
                return $sanitisedempids;
                ...
                if (!empty($searchField) && !empty($searchBy)
                    ...
                    if ($searchField == 'sub_unit') {
                       ...
                       // $conditions[] = " e.emp_number IN (SELECT erep_sup_emp_number FROM hs_hr_emp_reportto where erep_sub_emp_number = e.emp_number))";
                        ...
                        //die($searchBy."dsd");
                        if(!empty($filters['location']) && $filters['location'] != '-1' && $filters['location'] !="all"){
                            $conditions[] = ' l.location_id IN (' . $searchBy . ') ';
                        }
//                        else{
//                           $conditions[] = ' l.location_id like "%" ';  
//                        }
                    }
                    $filterCount++;
