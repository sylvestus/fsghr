...
class transferlistAction extends basePimAction {
    ...
    $offset=$request->getParameter('start');
          ...
          if($request->getParameter('location')&&$request->getParameter('fromdate')&&$request->getParameter('todate')){
             ...
             $this->employees =  AuditTrailTable::getTrailByModuleLocationTime("joblocation",$request->getParameter('location'),$request->getParameter('fromdate'),$request->getParameter('todate'),$offset, $limit);
         ...
         $dates=date("d/m/Y",strtotime($request->getParameter('fromdate')))." TO ".date("d/m/Y",strtotime($request->getParameter('todate')));
             ...
             $this->employees =  AuditTrailTable::getTrailByModuleLocationTime("joblocation",$request->getParameter('location'),"2016-01-01",date("Y-m-d"),$offset, $limit);
         $locationdao=new LocationDao();
         $locationdetails=$locationdao->getLocationById($request->getParameter('location'));
         $location=$locationdetails->name;
         $dates=date("d/m/Y",strtotime($request->getParameter('fromdate')))." TO ".date("d/m/Y",strtotime($request->getParameter('todate')));
           ...
           $this->employees =  AuditTrailTable::getTrailByModuleLocationTime("joblocation",NULL,$request->getParameter('fromdate'),$request->getParameter('todate'),$offset, $limit);
