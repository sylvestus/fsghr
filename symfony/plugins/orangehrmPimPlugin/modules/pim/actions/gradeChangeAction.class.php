 ...
 */
class gradeChangeAction extends basePimAction {
    ...
    $offset=$request->getParameter('start');
            ...
            if(!$page){$page=1;} if(!$limit){$limit=1000;} if(!$offset){$offset=0;}
          if($request->getParameter('location')&&$request->getParameter('fromdate')&&$request->getParameter('todate')){
              
             $this->employees =  AuditTrailTable::getTrailByModuleLocationTime("salarygrade",$request->getParameter('location'),$request->getParameter('fromdate'),$request->getParameter('todate'),$offset, $limit);
         ...
         $dates=date("d/m/Y",strtotime($request->getParameter('fromdate')))." TO ".date("d/m/Y",strtotime($request->getParameter('todate')));
           ...
           $this->employees =  AuditTrailTable::getTrailByModuleLocationTime("salarygrade",NULL,$request->getParameter('fromdate'),$request->getParameter('todate'),$offset, $limit);
       
         $dates=date("d/m/Y",strtotime($request->getParameter('fromdate')))." TO ".date("d/m/Y",strtotime($request->getParameter('todate')));
                   
             }
			 
			 else if($request->getParameter('location')&& empty($request->getParameter('fromdate')) && empty($request->getParameter('todate'))){
         ...
         $dates=date("d/m/Y",strtotime($request->getParameter('fromdate')))." TO ".date("d/m/Y",strtotime($request->getParameter('todate')));
              ...
              $location="All Locations";
             $this->employees =  AuditTrailTable::getTrailByModule("salarygrade",$offset, $limit);
          }
             $this->page=$page;
    }
    else{
