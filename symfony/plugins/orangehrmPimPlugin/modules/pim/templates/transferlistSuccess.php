...
<div id="recordsListDiv" class="box miniList">
    <div class="head">
        <h1><?php echo __('Transfer List'); ?>&nbsp;&nbsp; </h1>
    ...
    <div class="inner">
            ...
            <p id="listActions">
                <input type="hidden" id="filterurl" value="<?php echo url_for('pim/employeelist'); ?>">
                     ...
                     <select name="pageno" id="pagenumber">
                      ...
                      <form action="<?=  url_for('pim/transferlist')?>">
     ...
     <br>
			...
			<tr><td colspan="13" style="text-align:center"><?php echo '<img src="'.$path.'">'?></td></tr>
			<tr><td colspan="13" style="text-align:center"></td></tr>
                <tr><td colspan="13" style="text-align:center"> <u style="font-weight:bold">TRANSFER LIST <?=  strtoupper($locationd)?> FOR <?=$dates?></u>
                
                    </td></tr>
            <tr><td>#</td><td colspan="7">Number Of Entries: <?=count($employees)?></td><td colspan="3">Period: <?=date("F").",".date("Y")  ?></td><td colspan="3"><?=$page?></td></tr>
            
                  
                    <tr><td colspan="13" class="boldText" style="font-weight:bold"><center>Employer's Name: 
            <?php echo __(strtoupper($organisationinfo->getName())); ?></center></td></tr>
                    <tr>
                        <td class="boldText borderBottom" style="font-weight:bold" >#</td>
                        <td class="boldText borderBottom" style="font-weight:bold" ><?php echo __('No. H/MDA Code no.'); ?></td>
                        <td class="boldText borderBottom" style="font-weight:bold" ><?php echo __('Hay\'adda L W /From MDA\'s'); ?></td>
                        <td class="boldText borderBottom" style="font-weight:bold" ><?php echo __('NO H. la/MDA Code '); ?></td>
                         <td class="boldText borderBottom" style="font-weight:bold" ><?php echo __('Hay\'adda Lo W/To MDA\'s'); ?></td>
                       ...
                       <td class="boldText borderBottom" style="font-weight:bold"><?php echo __('T. La Qoary/Rec Date'); ?></td>
                       <td class="boldText borderBottom" style="font-weight:bold"><?php echo __('T. Wareejinta/Date of transfer'); ?></td>
           ...
           foreach ($employees as $record):  
                    ...
                    <tr>
                        <td class="tdName tdValue"><?=$count?></td>
                         ...
                         <?php $employeedetail=  EmployeeDao::getEmployeeByNumber($record["affected_user"]);
               $location = HsHrEmpLocationsTable::findEmployeeLocation($record["affected_user"]);
                ...
                $prevgrade=$dao->getPayGradeById($code);
                                 ?>
                             <?php $employee=EmployeeDao::getEmployeeByNumber($record["affected_user"]);
                              $loc= new LocationDao();
             $updatedlocation=$loc->getLocationById($postArray['job']['location']);
             $prevlocation=  explode("#",$record['previous_value']);
             $updatedloc=explode("#",$record['updated_value']);
                             ?>
                         
                            
                       <td class="tdName tdValue"><?=$prevlocation[0]?></td>
                        <td class="tdValue" style="text-align:right"><?=$prevlocation[1]?></td>
                         <td class="tdName tdValue"><?=$updatedloc[0]?></td>
                        <td class="tdValue" style="text-align:right"><?=$updatedloc[1]?></td>
                       ...
                       <?php if($employeedetail->emp_gender==2){echo "Female";}else{echo "Male";}?>    
                            ...
                            <td class="tdValue" style="text-align:right"><?=$prevgrade?></td>
                         <td class="tdValue" style="text-align:right"><?=date("d/m/Y",strtotime($employeedetail->joined_date))?></td>
                         <td class="tdValue" style="text-align:right"><?=date("d/m/Y",strtotime($record['dateupdated']))?></td>
                         <td class="tdValue" style="text-align:right"><a href='http://<?=$_SERVER['HTTP_HOST']?>/fgshr/symfony/web/uploads/<?=$record["file1"]?>' target="_blank">Ref</a></td>
...
$.ajax({
	...
	$('.print').click(function() {
     ...
     $('#pagenumber').change(function() {
		...
		window.location.replace("<?=url_for('pim/transferlist')?>"+"?limit=500&start="+offset+"&page="+page);
	}); 
