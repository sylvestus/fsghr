...
<div class="box pimPane">
    ...
    <div id="changeSalary">
        ...
        <div class="inner">
            ...
            <form id="frmSalary" action="<?php echo url_for('pim/viewSalaryList?empNumber=' . $empNumber); ?>" method="post" enctype="multipart/form-data" class="longLabels">
                <fieldset>
                    ...
                    <ol id="directDebitSection">
                        ...
                        <?php echo $directDepositForm['id']->render(); ?>
                            ...
                            <?php echo $directDepositForm['emp_number']->render(array("class" => "formInputText", "value"=>$empNumber, "maxlength" => 100)); ?>
                        </li>
                         <li>
                            <?php echo $directDepositForm['bankbranch']->renderLabel(__('Bank and Branch') . ' <em>*</em>'); ?>
                            <?php echo $directDepositForm['bankbranch']->render(array("class" => "formInputText", "maxlength" => 100)); ?>
                        </li>
                        <li>
                        ...
                        <li> 
                            ...
                            <?php echo $directDepositForm['amount']->render(array("class" => "formInputText", "maxlength" => 12,"value"=>0)); ?>
    ...
    <?php } ?>
    <input type="hidden" id="houseallowanceurl" value="<?=  url_for("pim/getEmpHouseAllowance")?>">
     <input type="hidden" id="updatetaxationurl" value="<?=  url_for("pim/updateTaxation")?>">
                            ...
                            <tr class="<?php echo $cssClass; ?>">
                                ...
                                <td>
                                    ...
                                    <input type="hidden" id="dd_id_<?php echo $salary->id; ?>" value="<?php echo $directDeposit->id; ?>" />
                                     <input type="hidden" id="dd_bankbranch_<?php echo $salary->id; ?>" value="<?php echo $directDeposit->bankbranch; ?>" />
                                                              <input type="hidden" id="emp_number_<?php echo $salary->id; ?>" value="<?php echo $directDeposit->emp_number; ?>" />
                                ...
                                <tr class="directDepositRow <?php echo $cssClass; ?>" style="display:none;">
                                    <td colspan="<?php echo $essUserMode || !$salaryPermissions->canDelete() ? '6' : '7'?>" class="<?php echo $cssClass; ?>" >
                                        ...
                                        <table class="table hover" style="width:60%">
                                            <thead>
                                                <tr>
                                                    <th><?php echo __("Bank and Branch"); ?></th>
                                            ...
                                            <tbody>
                                                <tr>
                                                    <td><?php $bankbranch=OhrmBankBranchTable::getBankFromBranch($directDeposit->bankbranch) ?>
                                                        <?=$bankbranch?>
                                                      </td>
   ...
   <div class="miniList" id="defaultpaymodediv">
             <div class="head">
            <h1><?php echo __("Default Payment Mode"); ?></h1>
        </div>  
        <div class="inner">
            <table id="tblSalarypay" class="table hover">
                  
                <tr><th>Default Payment Method</th><th><?php $defaultpay=HsHrEmpdefaultpaymodeTable::checkIfPaymentMethodExists($empNumber, $paymethod="");
                if(is_array($defaultpay)){
                    echo strtoupper($defaultpay["paymentmethod"]);
                }
                ?></th></tr></thead>
            </table>
            <br>
            <select id="defaultpaymode" class="greenselect">
           <option selected="selected">Change Payment Mode</option>
           <option value="cash">Cash</option>
           <option value="cheque">Cheque</option>
           <option value="bank">Bank</option>
       </select>  
             <input type="button" class="addbutton" id="btnSalaryPaymode" value="<?php echo __("Set Payment Mode"); ?>" />
       </div> 
        </div>  
    <!-- url for changing payment method -->
   <input type="hidden" id="paymentmethodurl" value="<?php echo url_for('pim/paymentMethod'); ?>"> 
    
   
    <div class="miniList" id="defaulttaxation">
             <div class="head">
            <h1><?php echo __("Taxation Setups"); ?></h1>
        </div>  
        <div class="inner">
            <table id="tblSalarypay" class="table hover">
                  <?php $employeedet=  EmployeeDao::getEmployeeByNumber($empNumber);
                  $carbenefit=$employeedet->vehicle_benefit;
                 $carvalue=$employeedet->vehicle_value; 
                  
                  if($employeedet->emp_pensionable>0){
                      $pension="YES";
                  }
                  else{
                      $pension="NO";
                  }
                  if($employeedet->pay_nssf>0){
                      $nssf="YES";
                  }
                  else{
                      $nssf="NO";
                  }
                   if($employeedet->pay_nhif>0){
                      $nhif="YES";
                  }
                  else{
                      $nhif="NO";
                  }
                   if($employeedet->pay_tax>0){
                      $tax="YES";
                  }
                  else{
                      $tax="NO";
                  }
                   if($employeedet->fixed_tax>0){
                      $fixedtax=$employeedet->fixed_tax;
                  }
                  
                   if($employeedet->emp_nick_name >0){
                      $insurancerelief=$employeedet->emp_nick_name;
                  }
                    if($employeedet->pay_relief>0){
                      $relief="YES";
                  }
                  else{
                      $relief="NO";
                  }
                   if($employeedet->encash_leave>0){
                      $encashleave="YES";
                  }
                  else{
                      $encashleave="NO";
                  ...
                  ?>
                <tr><th>TAXABLE</th><th><?=$tax?></th><th><select id="tax">
                            <option value="1">YES</option>
                            <option value="0">NO</option>
                        </select></th></tr>
                         <tr><th>FIXED TAX (%)</th><th><?=$fixedtax?></th><th><input type="text" name="fixedtax" value="<?=$fixedtax?>" id="fixedtax">
                            </th></tr>
                <tr><th> NSSF</th><th><?=$nssf?></th><th><select id="nssf">
                            <option value="1">YES</option>
                            <option value="0">NO</option>
                        </select></th></tr>
                <tr><th>NHIF</th><th><?=$nhif?></th>
                <th><select id="nhif">
                            <option value="1">YES</option>
                            <option value="0">NO</option>
                        </select></th>
                </tr>
                 <tr><th>PERSONAL RELIEF</th><th><?=$relief?></th><th><select id="relief">
                            <option value="1">YES</option>
                            <option value="0">NO</option>
                        </select></th></tr>
                 <tr><th>INSURANCE RELIEF (AMOUNT)</th><th><?=$insurancerelief?></th><th><input type="text" name="insurancerelief" value="<?=$insurancerelief?>" id="insurancerelief">
                <tr><th>PENSIONABLE</th><th><?=$pension?></th><th></th></tr>
                <tr><th>ENCASH LEAVE?</th><th><?=$encashleave?></th><th><select id="encashleave">
                            <option value="1">YES</option>
                            <option value="0" selected="selected">NO</option>
                        </select></th></tr>
                <tr><th>CAR BENEFIT?</th><th><select id="vehiclecapacity">
                            <option value="1200CC"  title="3600">Upto 1200cc</option>
                            <option value="1500CC" title="4200">Upto 1500cc</option>
                            <option value="1750CC" title="5800">1501cc-1750cc</option>
                            <option value="2000CC" title="7200">1751cc-2000cc</option>
                            <option value="3000CC" title="8600">2001cc-3000cc</option>
                            <option value="3001CC" title="14400">Over 3000cc</option>
                        </select></th><th><input type="text" name="carvalue" placeholder="Initial Vehicle Value" value="<?=$carvalue?>" id="carvalue">&nbsp;<input type="text" name="carbenefit" placeholder="Car Benefit" value="<?=$carbenefit?>" id="carbenefit">
                        </th> </table>
            <br>
             
             <input type="button" class="addbutton" id="btnSalaryTax" value="<?php echo __("Save Employee Configuration"); ?>" />
       </div> 
        </div>
...
//jquery
    ...
    //set payment mode
     $("#btnSalaryPaymode").click(function() {
        var paymode=$("#defaultpaymode").val();
       var url=$("#paymentmethodurl").val();
       window.location.replace(url+"?paymentmode="+paymode+"&empNumber="+<?php echo $empNumber ?>)
    ...
    //get house allowance automatically
//    $("#salary_salary_component").change(function(e){
//        selectedval=$("#salary_salary_component :selected").val();
//    if(selectedval=="House Allowance"){
//         url=$("#houseallowance").val()+"?empno=<?=$empNumber ?>";
//$.ajax({
//            url: url,
//            type: 'post',
//            data: {
...
//               $("#salary_basic_salary").val(success);
//            }
//        });  
//    }
//   });
   ...
   // });
     ...
     $("#btnSalaryTax").click(function(e){
       
 
         url=$("#updatetaxationurl").val()+"?empno=<?=$empNumber ?>";
$.ajax({
            url: url,
            type: 'post',
           
                data:{"nssf":$("#nssf :selected").val(),"nhif":$("#nhif :selected").val(),"tax":$("#tax :selected").val(),"relief":$("#relief :selected").val(),"fixedtax":$("#fixedtax").val(),"insurancerelief":$("#insurancerelief").val(),"encashleave":$("#encashleave :selected").val(),"carvalue":$("#carvalue").val(),"carbenefit":$("#carbenefit").val()},
...
$("#vehiclecapacity").change(function(e){
    $("#carbenefit").val("");$("#carvalue").val("");
    })

$("#carvalue").keyup(function(){
    var carbenefit=0;
    var benefit=($(this).val()*0.02);
    if(benefit > $("#vehiclecapacity :selected").attr("title")){  //prescribed
        carbenefit=benefit;
    }
    else{
        carbenefit=$("#vehiclecapacity :selected").attr("title");
    }
  $("#carbenefit").val(carbenefit);  //*$("#vehiclecapacity :selected").attr("title")
})
